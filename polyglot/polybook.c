...
static int hasPawnForCapture(const S_BOARD *pos) {
    int sqWithPawn = 0;
    int targetPce = (pos->side == WHITE) ? wP : bP;
    if (pos->enPas == NO_SQ)
        return FALSE;
    sqWithPawn = pos->side == WHITE ? pos->enPas - 10 : pos->enPas + 10;
    if(pos->pieces[sqWithPawn + 1] == targetPce || pos->pieces[sqWithPawn - 1] == targetPce) {
        return TRUE;
    }
    return FALSE;
...
u64 polykeyFromBoard(S_BOARD *board) {
    ...
    if(board->castlePerm & WSCA) finalKey ^= Random64Poly[offset];
    if(board->castlePerm & WLCA) finalKey ^= Random64Poly[offset+1];
    if(board->castlePerm & BSCA) finalKey ^= Random64Poly[offset+2];
    if(board->castlePerm & BLCA) finalKey ^= Random64Poly[offset+3];
